...
def test_ip_change():
    ...
    if not target_addr:
        ...
        print(f"\n[步骤3] 尝试添加新IP地址...")
        command = ['/ip/address/add', f'=address={new_ip}', f'=interface={target_interface}']
        ...
        try:
            ...
            if '!trap' in response:
                ...
                print(f"\n[步骤4] 验证修改结果...")
                api.write_sentence(['/ip/address/print', f'=.id={addr_id}'])
                    ...
                    if '!re' in resp:
                        ...
                        for line in resp:
                            if line.startswith('='):
                                parts = line[1:].split('=', 1)
                                if len(parts) == 2:
                                    verify_addr[parts[0]] = parts[1]
                        if verify_addr:
                            verify_response.append(verify_addr)
                
                if verify_response:
                    updated_addr = verify_response[0]
                    print(f"修改后的IP地址信息:")
                    print(f"  ID: {updated_addr.get('.id')}")
                    print(f"  IP: {updated_addr.get('address')}")
                    print(f"  接口: {updated_addr.get('interface')}")
                    
                    if updated_addr.get('address') == new_ip:
                        print(f"\n✅ 验证成功: IP地址已正确修改为 {new_ip}")
                        return True
                    else:
                        print(f"\n❌ 验证失败: IP地址不匹配")
                        print(f"  期望: {new_ip}")
                        print(f"  实际: {updated_addr.get('address')}")
                        return False
                else:
                    print(f"\n⚠️ 无法验证修改结果")
                    return True
