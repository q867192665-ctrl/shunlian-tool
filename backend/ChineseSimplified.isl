...
;
; Note: When translating this file, do not add periods (.) to the end of
...
AboutSetupNote=
ConfirmDeleteSharedFile2=文件"%1"正被其他程序使用。%n%n您要删除此文件吗？
ConfirmDeleteSharedFileTitle=确认删除共享文件
ConfirmUninstall=您确定要完全卸载 %1 吗？%n%n%n如果确定，请点击"是"继续。
ErrorOpeningReadme=打开自述文件时出错。
ErrorRestartingComputer=安装程序无法重启计算机。
OnlyAdminCanUninstall=只有管理员才能卸载此程序。
SharedFileNameLabel=共享文件名：
SharedFileLocationLabel=共享文件位置：
ShutdownBlockReasonInstallingApp=正在安装 %1
ShutdownBlockReasonUninstallingApp=正在卸载 %1
StatusUninstalling=正在卸载...
UninstallDataCorrupted=卸载数据已损坏，无法继续卸载。
UninstalledAll= %1 已从您的计算机中完全卸载。
UninstalledAndNeedsRestart= %1 的大部分已从您的计算机中卸载，但需要重启计算机才能完成卸载。%n%n要立即重启吗？
UninstalledMost= %1 的大部分已从您的计算机中卸载。
UninstallNotFound=卸载数据"%1"不存在。无法继续卸载。
UninstallOnlyOnWin64=此程序只能在 64 位 Windows 上卸载。
UninstallOpenError=打开卸载数据文件时出错。可能文件已损坏或被其他程序锁定。
UninstallStatusLabel=正在从您的计算机中删除 %1，请稍候。
UninstallUnknownEntry=卸载数据中存在未知的条目：%1
UninstallUnsupportedVer=此卸载程序不支持此版本的 Windows。无法继续卸载。
